...
const NonVerified = () => {
  ...
  const fetchNonVerifiedUsers = () => {
    fetch("https://testnetback.s39global.com/api/user-panel/unverified")
      .then((res) => res.json())
      .then((data) => {
  ...
  const deleteMultipleMerchants = () => {
    ...
    fetch("https://testnetback.s39global.com/api/user-panel/multi-delete", {
      method: "DELETE",
      headers: {
  ...
  const verifyUser = (id) => {
    swal({
      ...
      if (willDelete) {
        axios
          .put(
            `https://testnetback.s39global.com/api/user-panel/admin/verify/${id}`
          )
          .then((res) => {
            if (res.status === 200) {
              swal({
      ...
      <div className="text-white row align-items-center g-2">
        <div className="col-12 col-lg-6">
          <CSVLink data={nonVrifiedCsv} className="mt-4">
            <button className="my-3 text-white bolder  bg-primary p-2 my-2 rounded border-0">
              GENNERATE CSV
            </button>
          </CSVLink>
          <button
            ...
            className={`${selectedUserIds.length === 0 ? "btn-secondary" : "btn-danger"
              } text-white bolder p-2 px-2 ml-3 rounded border-0 btn`}
          >
            DELETE
          </button>
        </div>
              ...
              <tr style={{ borderBottom: "1px solid white" }} className=" ">
                ...
                <td className="text-start">
                  <Tooltip title="View user details" placement="top">
                    <Link to={`/admin/non-verified/${d.walletAddress}`}>
